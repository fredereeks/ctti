        ...
        return (
            <li className={`relative group`}>
                <div className={`flex items-center w-full cursor-pointer hover:bg-cyan-400 md:hover:bg-transparent group`}>
                    <Link href={link} className={`group-hover:text-white md:group-hover:text-slate-700 font-medium ${activeLink} text-base py-2 px-[.65rem] text-slate-600 hover:text-white hover:bg-cyan-400 md:hover:bg-transparent md:hover:text-slate-700 flex gap-1 items-center whitespace-nowrap`}>{title}</Link>
                ...
                {sublinks?.map(({id,link,title}) => (
                    <li key={id} className={`relative`}>
                        <Link href={link} key={id} className={`text-base py-2 px-3 text-slate-600 font-medium hover:text-white hover:bg-primary  flex gap-1 items-center`}>{title}</Link>
                    </li>
                ))}
                </ul>
            </li>
